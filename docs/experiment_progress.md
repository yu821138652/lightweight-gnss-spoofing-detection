...
## 7. 多环境联合训练：4 折静态 Session-CV

在 [static_session_cv_4fold](protocols/static_session_cv_4fold/) 中，每折静态 train 同时包含新主楼与操场 Session，静态 val/test 也各保留两个环境的未见 Session。`static+dynamic` 对照使用完全相同的静态 split，仅额外向 train 加入 10 个固定动态 Session。

**该节是“动态训练是否帮助静态检测”的消融实验，val/test 均为静态；不等同于下节的统一静态+动态检测模型。**

| 训练方案 | test Macro-F1（4 折均值 +/- 标准差） | Recall | FAR | 结论 |
|---|---:|---:|---:|---|
| 多环境 static-only | 0.8964 +/- 0.0693 | 78.71% +/- 21.92% | 2.37% +/- 4.05% | 多环境静态联合训练已降低单一环境依赖，但不同 held-out Session 间波动仍明显。 |
| [多环境 static+dynamic](protocols/static_dynamic_train_cv_4fold/README.md) | **0.9169 +/- 0.0659** | **82.69% +/- 20.14%** | **1.96% +/- 2.93%** | 动态训练使平均 F1 提升 0.0206、Recall 提升 3.98 个百分点、FAR 降低 0.41 个百分点。 |

### LightGBM L=30 训练协议总表

下表只比较同一 27 维设备统计特征、因果窗口 `L=30` 和默认阈值 0.5 下的 LightGBM。单次 test 结果用于开发诊断；只有 4 折结果可作为当前较稳健的开发评估。

| 协议 | 训练数据 | 静态 test | Macro-F1 | Recall | FAR | 结果边界 |
|---|---|---|---:|---:|---:|---|
| 静态跨环境开发参考 | 新主楼静态 | 操场静态 | 0.9161 | 79.08% | 0.29% | test 已参与开发，不是最终独立测试。 |
| 新主楼同环境 static-only | 新主楼静态 | 新主楼未见静态 Session | 0.9689 | 99.80% | 4.15% | 单次划分，验证集只有 1 个设备。 |
| 新主楼同环境 static+dynamic | 新主楼静态 + 动态 | 与上一行相同 | 0.9838 | 99.88% | 2.13% | 与上一行可直接比较。 |
| 操场同环境 static-only | 操场静态 | 操场未见静态 Session | 0.7668 | 48.89% | 0.00% | 单次划分，L5 验证集使模型过于保守。 |
| 操场同环境 static+dynamic | 操场静态 + 动态 | 与上一行相同 | 0.9944 | 98.87% | 0.17% | 与上一行可直接比较。 |
| 多环境 static-only，4 折 CV | 两环境静态 | 两环境未见静态 Session | 0.8964 +/- 0.0693 | 78.71% +/- 21.92% | 2.37% +/- 4.05% | 当前静态多环境联合训练基线。 |
| 多环境 static+dynamic，4 折 CV | 两环境静态 + 10 个动态 train Session | 与上一行相同 | **0.9169 +/- 0.0659** | **82.69% +/- 20.14%** | **1.96% +/- 2.93%** | 当前最完整的静态开发对照。 |

## 8. 统一静态+动态检测：混合场景固定 Session 划分

统一模型使用 `output/tensors_mixed/recording_split_manifest.csv`：静态与动态均进入 train、val、test，且完整 Session 不跨 split。设备级 `LightGBM L=30` 使用 27 维统计特征和默认阈值 0.5。

| 测试子集 | Macro-F1 | Recall | FAR | 窗口数 |
|---|---:|---:|---:|---:|
| 总体静态+动态 test | 0.7944 | 51.33% | 0.51% | 11,943 |
| 静态 test 子集 | 0.8118 | 54.75% | 0.29% | 8,159 |
| 动态 test 子集 | 0.7500 | 43.00% | 0.98% | 3,784 |

该模型已能够同时对静态和动态窗口输出设备级告警，但动态 Recall 仍低于静态。此表与第 7 节的纯静态 test 结果不可直接横比，因为测试分布、Session 组成和任务目标不同；它是当前统一检测任务的首个基线。

## 9. 下一阶段

1. 导出 `LightGBM L=30` 的特征贡献，分析特征组和历史时刻贡献。
2. 构造因果相对特征：相对滚动基线、变化率、短期波动、分位差等。
3. 在同一多环境 4 折协议上复验 `DLinear L=30` 与 `MLP L=5`，判断动态训练增益是否依赖 LightGBM。
4. 对统一静态+动态模型构建多折 Session-CV，并分别报告静态、动态与总体指标。
5. 使用未参与动态训练的动态 Session，分别测试 static-only 与 static+dynamic 的静态到动态迁移。
6. 增加跨设备指标、TTD、虚警频率、模型大小、CPU/端侧时延；教师模型和知识蒸馏仅在上述基础完成后进行。
