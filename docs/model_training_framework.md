...
> 本文保留张量接口和训练边界的通用说明。当前项目状态与已知数据问题以 [handoff_status.md](handoff_status.md) 为准；P0–P5 设备级模型和本文中的旧 baseline 不代表已锁定最终模型。
...
signal_id
  -> 窗口末端存在的信号槽位，每条信号有一个当前历元的二分类标签
```

每个样本使用截至当前时刻的连续 5 个历元作为因果上下文，预测第 5 个、即窗口末端历元的 `Label`。只有窗口末端实际观测到的 `signal_id` 参与损失和指标计算；前 4 个历元仅提供历史特征。这样攻击结束后不会因窗口内早先存在欺骗样本而把当前正常时刻继续标为欺骗。
...
现有 `pipeline_total/08_inference.py` 是历史模型接口，尚不能加载本文件定义的 baseline checkpoint。在设备级聚合规则确定前，不应使用它对 CSV 生成部署结论。训练张量或窗口标签语义重建后，此前导出的 baseline checkpoint 与验证/测试指标均不再可比，必须在新张量上重新训练并重新评估。
