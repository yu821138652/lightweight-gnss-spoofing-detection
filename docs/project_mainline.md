...
第一篇已经在 FGI-GSRx / Finland 数据上完成了富特征条件下的高精度欺骗与干扰检测，使用的软件接收机特征较丰富，例如 I/Q、相关能量、跟踪环误差、FLL/PLL/DLL 等。

第二篇如果继续在类似数据上追求更高精度，增量不够明显。更有价值的方向是转向真实部署场景：
