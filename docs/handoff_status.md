...
## 最新进展：频段均值场景四分类新路线（2026-08-01）

> 这是一条**新增的并行路线**，不替代下面的逐 `signal_id` 主线。完整过程记录见 [频段均值场景四分类](band_mean_scene_classification_20260729.md)。

### 一句话结论

把判别单元从"逐 `signal_id` 是否被欺骗"改为"**当前时刻属于哪种欺骗场景**"（正常/L1/L5/L1+L5 四分类），输入改为每历元按频段取特征均值的紧凑窗口向量。**只保留 C/N0 家族**时，静态跨录制 Macro-F1 **0.992**，含动态的 mixed 范围 **0.922**。模型 5348 参数（tcn）。

### 已确认事实

| 实验 | 协议 | 维度 | Macro-F1 |
|---|---|---:|---:|
| 初版（全部 4 特征 × 2 频段 + presence） | static 7 折留一 | 10 | 0.728 |
| 去 AGC | static 7 折留一 | 8 | 0.948 |
| **只留 C/N0（无差值，当前最佳）** | static 7 折留一 | **4** | **0.992** |
| 只留 C/N0 + 差值 | static 7 折留一 | 5 | 0.987 |
| 只留 C/N0 + 差值 | **mixed 4 折 grouped CV** | 5 | **0.922** |

- **核心发现**：在这条路线上，**只有 C/N0 能跨录制迁移**。AGC、接收机时间不确定度、伪距率不确定度三者单独去掉后 Macro-F1 都上升（分别 +0.220 / +0.163 / +0.129），只有去掉 C/N0 会变差（−0.011）。它们是**设备指纹**而非攻击特征。
- **AGC 的机制已定位**：RedMi_K60 的 AGC 全程仅 6 个离散值（MI8 有 705 个），`AgcDbMissing=0` 说明是硬件特性而非缺失。基线模型学到"AGC 下掉 ⟺ L1 被打"的捷径，换录制即失效。去 AGC 后 RedMi_K60 攻击窗口 recall 从 **0.228 → 0.996**。
- **动态未重演主线瓶颈**：主线逐信号二分类的 `dy_L5` recall 长期 ~18.7%，而本路线 mixed 范围下 L5（含动态）recall **0.877**、L1+L5 **0.890**。L5 与 L1+L5 之间几乎不混（L5→L1+L5 仅 1 个）。
- **差值特征是负结果**：显式加 `L1_Cn0 − L5_Cn0` 净贡献 **−0.005**（0.987 vs 0.992）。它是两维 C/N0 的线性组合，模型本可自学。在 full 配置下有用（+0.115），毒特征砍掉后作用被覆盖。builder 默认生成该列，当前推荐用 `--drop-features Cn0DbHzL1MinusL5` 关闭。
- **类权重再平衡是负结果**：L1/L1+L5 权重 ×2.5 后 Macro-F1 从 0.728 降到 0.699，L1 recall 反降（0.475→0.384）。L1 判不准的根源是特征不可迁移，不是阈值保守。

### 重要边界条件（避免误用）

**"AGC 是毒特征"不能推广到主线二分类。** 严格复刻主线 `state_stratified_v2` 协议、同样 raw+stats 双分支与 TCN16，只把特征换成只用 C/N0：

| 指标 | 主线 compact11 | cn0_only | 变化 |
|---|---:|---:|---:|
...
原因：四分类判"**哪个**频段被打"，依赖跨设备可比的相对关系，AGC 的设备不可比性构成污染；二分类只判"**有没有**被打"，AGC 的绝对下掉是很强的本地证据，每个信号与自身历史比较，设备间不可比不构成问题。

### 方法论提醒

- **消融重训是硬证据，置换重要性是辅助**，两者测的问题不同。置换测"已训成的模型多依赖某特征"，消融测"该特征不存在时会怎样"。
- 置换脚本**必须多次重复取均值**。初版单次置换导致 AGC 贡献符号在两次运行间翻转（+0.09 / −0.06）；改为 20 次重复后稳定（std ≤ 0.004）。
- 本路线四分类 Macro-F1 与主线二分类 0.8637 **不是同一任务**，禁止直接比较。

### 局限与下一步

- **single-band 窗口被隔离**：端点历元只观测到单一频段的窗口（static 23473 个）标签强制为 0 且排除出全部统计，留给后续单独路径。它们占 static test 的 45%，是尚未被这条路线覆盖的部分。
- **未做**：window 长度扩展（W=15/30，针对"窗口全泡在攻击段内看不到跳变"这一诊断出的根因）；参照基线偏移特征；per-device AGC 可信度门控。
- **张量目录**：`output/tensors/band_mean_window_static_v2`（static，11 维含差值）、`output/tensors/band_mean_window_mixed_v2`（mixed）。训练产物在 `output/training/band_mean_multiclass_cv*`。
