...
- 阶段 0：基线与协议冻结，已完成。
- 阶段 1：设备响应分支特征消融，已完成主要对照，当前正式候选为 44 维 C/N0-extreme + MLP-h16 + direct expert。
- 阶段 2：设备响应分支模型规模比较，已完成主要 Linear/MLP-h8/h16/h32 对照，MLP-h16 是当前综合候选。
- **阶段 3：场景分支特征优化，已完成。** S1 四维 C/N0-only + TCN 为正式场景候选。
- **阶段 4：场景分支模型比较，进行中。** 下一步固定 S1 输入，比较 TCN、GRU、LSTM 和轻量 1D CNN。
- 阶段 5：后融合优化，待阶段 4 完成后进行。
- 阶段 6：实时性与轻量化，待最终模型候选冻结后进行。

阶段 3 之后不得同时改变场景特征集合和模型结构；阶段 4 的比较必须在相同六折协议、窗口长度和 S1 四维输入下进行。
