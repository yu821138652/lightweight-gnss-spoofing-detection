# 场景门控的设备响应联合诊断：静态六折完整方案

> 状态：当前静态数据上的最终候选方案（开发性六折结果）。本文档记录截至 2026-08-06 的实现、结果和边界；原始日志、张量、checkpoint 与预测文件均只保存在本地 `output/`，不纳入 Git。
...
- 当前结论仅针对静态六折开发性评估；动态泛化、TTD（从攻击开始到稳定告警的时延）以及端到端推理耗时/参数量仍需作为后续独立实验完成。
