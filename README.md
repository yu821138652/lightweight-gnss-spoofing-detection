...
- 已生成 4 折静态 Session 交叉验证清单，下一阶段将围绕相对特征、跨设备/跨环境评估和检测时延开展复验。
...
3. 在锁定的新特征协议上运行 4 折静态 Session-CV，并报告均值和标准差。
...
当前优先解决跨环境泛化，而不是继续机械增加模型：先完成特征贡献分析与因果相对特征重建，再在 4 折 Session-CV、跨设备和动态场景协议上复验 `LightGBM L=30`、`DLinear L=30` 与低虚警 `MLP L=5`。
